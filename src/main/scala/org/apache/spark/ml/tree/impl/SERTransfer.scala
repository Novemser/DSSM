...
object SERTransfer extends ModelTransfer {
  def transferModels(
    ...
    val nodesToTransfer = mutable.ArrayBuffer[TransferLearningNode]()
//    println(s"B4:${trainedModels.head.rootLearningNode.toNode.subtreeToString()}")
    topNodes.zipWithIndex.foreach(t => {
      extractAllNodes(t._1).foreach(leaf => {
        nodeStack.push((t._2, leaf))
        nodesToTransfer.append(leaf)
      })
    })
    timer.stop("init")
    println(s"nodesToTransfer.size:${nodesToTransfer.length}")
...
//    println(s"after:${trainedModels.head.rootLearningNode.toNode.subtreeToString()}")
  ...
  def transfer(trainedModel: RichDecisionTreeClassificationModel,
    ...
    val nodesToTransfer = topNodes.flatMap(extractAllNodes)
    nodesToTransfer.foreach(node => nodeStack.push((0, node)))
    timer.stop("init")
    ...
    // Pruning
    nodesToTransfer.foreach(nodeWithTgtError => {
      // If the leaf error of current node is smaller than the expanded sub-tree,
      // that means that if we do not expand current node, will get better result
      // in the target dataset.
      if (nodeWithTgtError.leftChild.nonEmpty) {
        val leafError = Utils.leafError(nodeWithTgtError)
        val subTreeError = Utils.subTreeError(nodeWithTgtError)
        println(s"LeafErr:$leafError, SubTreeError:$subTreeError")
        if (leafError < subTreeError) {
          // Pruning this node
          logInfo(s"Pruning node [${nodeWithTgtError.id}]")
          nodeWithTgtError.leftChild = None
          nodeWithTgtError.rightChild = None
          nodeWithTgtError.isLeaf = true
        }
      }
    })
    println("Pruned tree:\n" + trainedModel.rootLearningNode.toNode.subtreeToString())
  ...
  private def extractAllNodes(node: TransferLearningNode): Array[TransferLearningNode] = {
    ...
    Array(node) ++
      extractAllNodes(node.leftChild.get.asInstanceOf[TransferLearningNode]) ++
      extractAllNodes(node.rightChild.get.asInstanceOf[TransferLearningNode])
  }
...
//        logWarning(s"Calculating nodes ${nodes.map(_.id).mkString(",")}")
      ...
      case (treeIndex, nodesForTree) =>
        nodesForTree.foreach { node =>
          ...
          if (node.leftChild.nonEmpty && node.rightChild.nonEmpty) {
            // if there is any target transfer data arrives and original node should be preserved
            // no need to iterate over child nodes
          } else {
            // update info used for prediction
            node.stats = stats
            node.split = Some(split)
            if (isLeaf) {
              // this node does not have any target transfer data arrives, prune the node
              node.leftChild = None
              node.rightChild = None
            } else {
//              logWarning(s"Expanding node ${node.id}")
              val leftChildIsLeaf = stats.leftImpurity == 0.0
              val rightChildIsLeaf = stats.rightImpurity == 0.0
              node.leftChild = Some(
              ...
              if (nodeIdCache.nonEmpty) {
                val nodeIndexUpdater = NodeIndexUpdater(split = split, nodeIndex = nodeIndex)
  ...
  private def binsToBestSplit(binAggregates: DTStatsAggregator,
    ...
    var gainAndImpurityStats: ImpurityStats = if (node.isLeaf) null else node.stats
  ...
  def mixedBinSeqOp(agg: DTStatsAggregator,
                    treePoint: TreePoint,
                    splits: Array[Array[Split]],
