...
object STRUTTransfer extends ModelTransfer {
  override def transferModels(trainedModels: Array[RichDecisionTreeClassificationModel],
                              ...
                              parentUID: Option[String]): Array[RichDecisionTreeClassificationModel] = {
    ...
    while (nodeStack.nonEmpty) {
      ...
      calculateNewSplitsStats(baggedInput, metadata, topNodesForGroup, nodesForGroup,
  ...
  private def calculateNewSplitsStats(input: RDD[BaggedPoint[TreePoint]],
    ...
    val nodeNewStatsMap = partitionAggregates.reduceByKey((a, b) => a.merge(b)).map {
          ...
          val (bestFeatureSplitIndex, bestFeatureGainStats, bestSplitError) =
            Range(0, numSplits).map { splitIdx =>
              ...
              val splitDataStats = Array
                .tabulate(lStats.length) { idx => lStats(idx) + rStats(idx) }

              gainAndImpurityStats = calculateImpurityStats(gainAndImpurityStats,
                leftChildStats, rightChildStats, binAggregates.metadata)
              val parentError = Utils.calcClassificationError(splitDataStats, gainAndImpurityStats.impurityCalculator.predict)
  ...
  private def extractNodes(node: TransferLearningNode,
    ...
    if (node.leftChild.isEmpty && node.rightChild.isEmpty) {
      ...
      Array(node) ++
        extractNodes(node.leftChild.get.asInstanceOf[TransferLearningNode],
          nodeStatsInfoMap,
          invalidateStats) ++
        extractNodes(node.rightChild.get.asInstanceOf[TransferLearningNode],
          nodeStatsInfoMap,
          invalidateStats)
    }
  }
