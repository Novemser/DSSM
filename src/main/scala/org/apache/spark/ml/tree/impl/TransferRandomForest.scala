...
object TransferRandomForest extends Logging {
  ...
  def run(input: RDD[LabeledPoint],
    ...
    val topNodes =
      Array.fill[TransferLearningNode](numTrees)(TransferLearningNode.emptyNode(nodeIndex = 1))
    ...
    parentUID match {
      ...
      case None =>
        if (strategy.algo == OldAlgo.Classification) {
          ...
          topNodes.map(
            rootNode => new RichDecisionTreeRegressionModel(rootNode.toNode, numFeatures, rootNode)
          )
        }
    }
  }
    ...
    def binSeqOp(agg: Array[DTStatsAggregator], baggedPoint: BaggedPoint[TreePoint]): Array[DTStatsAggregator] = {
      ...
      treeToNodeToIndexInfo.foreach {
        case (treeIndex, nodeIndexToInfo) =>
          ...
          //          println(s"current tree:\n${topNodesForGroup(treeIndex).toNode.subtreeToString()}")
